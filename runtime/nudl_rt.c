...
void __nudl_array_push(void *arr_ptr, int64_t value) {
  ...
  if (arr->length >= arr->capacity) {
    ...
    int64_t *new_buf =
        (int64_t *)realloc(old_buf, (size_t)new_cap * sizeof(int64_t));
    if (!new_buf) {
...
int64_t __nudl_array_get(void *arr_ptr, int64_t index) {
  ...
  if (index < 0 || index >= arr->length) {
    fprintf(stderr,
            "nudl: array index out of bounds: index %lld, length %lld\n",
...
/* Remove element at index and return it. Shifts remaining elements down. Panics
 * on OOB. */
 ...
 * is_ref_elem: 1 if elements are reference types that need ARC release, 0
 * otherwise. elem_drop: drop function for reference-typed elements (NULL if no
 * custom Drop). */
void __nudl_array_destroy(void *arr_ptr, int64_t is_ref_elem,
                          void (*elem_drop)(void *)) {
  NudlDynArray *arr = (NudlDynArray *)arr_ptr;
  int64_t *buf = (int64_t *)(uintptr_t)arr->data_ptr;
  if (buf) {
...
void *__nudl_map_alloc(void) {
  ...
  NudlMapEntry *entries =
      (NudlMapEntry *)calloc((size_t)initial_cap, sizeof(NudlMapEntry));
  if (!entries) {
...
static void nudl_map_grow(NudlMap *map) {
  ...
  NudlMapEntry *new_entries =
      (NudlMapEntry *)calloc((size_t)new_cap, sizeof(NudlMapEntry));
  if (!new_entries) {
...
void *__nudl_str_concat(const char *a_ptr, int64_t a_len, const char *b_ptr,
                        int64_t b_len) {
  ...
  *len_field = total_len;
  ...
  if (a_ptr && a_len > 0)
    memcpy(data, a_ptr, (size_t)a_len);
  if (b_ptr && b_len > 0)
    memcpy(data + a_len, b_ptr, (size_t)b_len);
  data[total_len] = '\0';
  return mem;
}
...
/* Get byte at index from a string (ptr, len pair). Returns as i64. Panics on
 * OOB. */
int64_t __nudl_str_char_at(const char *ptr, int64_t len, int64_t index) {
  if (index < 0 || index >= len) {
    fprintf(stderr,
            "nudl: string index out of bounds: index %lld, length %lld\n",
...
void *__nudl_str_substr(const char *ptr, int64_t len, int64_t start,
                        int64_t end) {
  if (start < 0)
    start = 0;
  if (end > len)
...
int64_t __nudl_str_indexof(const char *h_ptr, int64_t h_len, const char *n_ptr,
                           int64_t n_len) {
  if (n_len == 0)
    return 0;
  if (n_len > h_len)
...
int64_t __nudl_str_contains(const char *h_ptr, int64_t h_len, const char *n_ptr,
                            int64_t n_len) {
  return __nudl_str_indexof(h_ptr, h_len, n_ptr, n_len) >= 0 ? 1 : 0;
}

/* Check if string starts with prefix. Returns 1 or 0. */
int64_t __nudl_str_starts_with(const char *ptr, int64_t len, const char *p_ptr,
                               int64_t p_len) {
  if (p_len > len)
    return 0;
...
int64_t __nudl_str_ends_with(const char *ptr, int64_t len, const char *s_ptr,
                             int64_t s_len) {
  if (s_len > len)
    return 0;
...
void *__nudl_str_replace(const char *ptr, int64_t len, const char *old_ptr,
                         int64_t old_len, const char *new_ptr,
                         int64_t new_len) {
  if (old_len == 0)
    return __nudl_str_alloc(ptr, len);
  ...
  void *mem = malloc((size_t)alloc_size);
  if (!mem) {
    fprintf(stderr, "nudl: out of memory (str_replace)\n");
    abort();
  }
  NudlArcHeader *hdr = (NudlArcHeader *)mem;
  hdr->strong_count = 1;
