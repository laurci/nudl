use miette::{self, SourceSpan, MietteDiagnostic as MietteReport, Report, NamedSource};
...
fn render_one(report: &DiagnosticReport, source_map: &SourceMap) {
    if report.labels.is_empty() {
        ...
        eprintln!("{}: [E{:04}] {}", severity_str, report.info.code, report.message);
        return;
    }
    ...
    for label in &report.labels {
        ...
        let diag = MietteReport::new(format!(
            "[E{:04}] {}",
            report.info.code,
            report.message
        ))
